const sub_categories_1_controller = require("../controller/sub_categories_1");
...
const createSubCategories = async (userData) => {
    try {
        ...
        if (decodedData.role == "admin") {
            ...
            const response = await sub_categories_1_controller.createSubCategories(userData.body);
...
const updateSubCategories = async (userData) => {
    try {
        ...
        if (decodedData.role == "admin") {
            ...
            if (userData.file) {
                const getUserDate = await sub_categories_1_controller.getSubCategoriesById(userData.body.Sub_Cat_Level_1_Id)
            ...
            const updatedData = await sub_categories_1_controller.updateSubCategories(userData.body.Sub_Cat_Level_1_Id, userData.body)
...
const deleteSubCategories = async (userData) => {
    try {
        ...
        if (decodedData.role == "admin") {
            const deletedData = await sub_categories_1_controller.deleteSubCategories(userData.body.Sub_Cat_Level_1_Id,)
...
const getSubCategoriesById = async (userData) => {
    try {
        ...
        if (decodedData.role == "admin") {
            const response = await sub_categories_1_controller.getSubCategoriesById(userData.body.Sub_Cat_Level_1_Id)
            return successResponse(response)
        }
    } catch (error) {
...
const getAllSubCategories = async (userData) => {
    try {
        ...
        // }
        // if (decodedData.role == "admin") {
            const response = await sub_categories_1_controller.getAllSubCategories(userData.body)
            return successResponse(response)
        // }
...
const searchSubCategoriesDetails = async (userData) => {
    try {
        ...
        if (decodedData.role == "admin") {
            const Response = await sub_categories_1_controller.searchSubCategoriesDetails(userData.body)
            return successResponse(Response)
        }
        return errorResponse("access denied...!")
    } catch (error) {
                ...
                .on('data', (row) => {
                    ...
                    if (hasData) {
                        ...
                        if (!row.Sub_Cat_Level_1_Id || row.Sub_Cat_Level_1_Id.trim() === '') {
                            row.Sub_Cat_Level_1_Id = null; // This will allow auto-increment to work
                        }
                ...
                .on('end', async () => {
                    try {
                        for (const category of categories) {
                            ...
                            if (category.Sub_Cat_Level_1_Id && category.Sub_Cat_Level_1_Id.trim() !== '') {
                                // If it exists, update the category
                                await sub_categories_1_controller.updateOrCreateCategory(category.Sub_Cat_Level_1_Id, category);
                            } else {
                                // If Id is not present, create a new category
                                await sub_categories_1_controller.createSubCategories(category);
                            }
                        }
