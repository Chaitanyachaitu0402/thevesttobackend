const sub_categories_controller = require("../controller/sub_categories_3");
...
const updateSubCategories = async (userData) => {
    try {
        ...
        if (decodedData.role == "admin") {
            ...
            if (userData.file) {
                const getUserDate = await sub_categories_controller.getSubCategoriesById(userData.body.Sub_Cat_Level_3_Id)
            ...
            const updatedData = await sub_categories_controller.updateSubCategories(userData.body.Sub_Cat_Level_3_Id, userData.body)
...
const deleteSubCategories = async (userData) => {
    try {
        ...
        if (decodedData.role == "admin") {
            const deletedData = await sub_categories_controller.deleteSubCategories(userData.body.Sub_Cat_Level_3_Id,)
...
const getSubCategoriesById = async (userData) => {
    try {
        ...
        if (decodedData.role == "admin") {
            const response = await sub_categories_controller.getSubCategoriesById(userData.body.Sub_Cat_Level_3_Id)
            return successResponse(response)
        }
    } catch (error) {
                ...
                .on('data', (row) => {
                    ...
                    if (hasData) {
                        ...
                        if (!row.Sub_Cat_Level_3_Id || row.Sub_Cat_Level_3_Id.trim() === '') {
                            row.Sub_Cat_Level_3_Id = null; // This will allow auto-increment to work
                        }
                ...
                .on('end', async () => {
                    try {
                        for (const category of categories) {
                            ...
                            if (category.Sub_Cat_Level_3_Id && category.Sub_Cat_Level_3_Id.trim() !== '') {
                                // If it exists, update the category
                                await sub_categories_controller.updateOrCreateCategory(category.Sub_Cat_Level_3_Id, category);
                            } else {
