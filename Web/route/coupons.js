...
router.post('/update-coupon-by-code', async (req, res) => {
    const response = await couponService.updateCouponByCode(req);
    if (response.success) {
        res.json(response)
    } else {
        res.status(500).json(response);
...
router.post('/delete-coupon-by-name', async (req, res) => {
    const response = await couponService.deleteCouponByCode(req);
    if (response.success) {
        res.json(response)
    } else {
        res.status(500).json(response);
