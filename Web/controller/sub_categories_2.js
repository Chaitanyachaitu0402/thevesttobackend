...
const sub_categoriestModel = require('../../model/sub_categories_2')(sequelize)
...
const updateSubCategories = async (Sub_Cat_Level_2_Id, userData) => {
    try {
        const newUser = await sub_categoriestModel.update(userData, {
            where: {
                Sub_Cat_Level_2_Id: Sub_Cat_Level_2_Id
            }
        });
        return newUser;
    }
...
const deleteSubCategories = async (Sub_Cat_Level_2_Id) => {
    try {
...
const getSubCategoriesById = async (Sub_Cat_Level_2_Id) => {
    try {
...
const searchSubCategoriesDetails = async (userData) => {
    try {
        ...
        if (userData.Sub_Cat_Level_2_Id) {
            data.Sub_Cat_Level_2_Id = userData.Sub_Cat_Level_2_Id
        }
...
const updateOrCreateCategory = async (Sub_Cat_Level_2_Id, categoryData) => {
    try {
        // Check if the category exists
        const existingCategory = await sub_categoriestModel.findOne({
            where: { Sub_Cat_Level_2_Id: Sub_Cat_Level_2_Id }
        });
        ...
        if (existingCategory) {
            ...
            return { message: `Category with Id ${Sub_Cat_Level_2_Id} updated.` };
        } else {
            // If not exists, create a new category
            const newCategory = await sub_categoriestModel.create(categoryData);
            return { message: `New category created with Id ${newCategory.Sub_Cat_Level_2_Id}` };
        }
    } catch (error) {
